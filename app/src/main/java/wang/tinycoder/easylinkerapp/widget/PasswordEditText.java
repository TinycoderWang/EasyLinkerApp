package wang.tinycoder.easylinkerapp.widget;
 ...
 * Package：wang.tinycoder.easylinkerapp.widget
 * Desc：可显示/隐藏文本信息的EditText
 * Author：TinycoderWang
 * CreateTime：2018/4/1 7:47
 */
public class PasswordEditText extends AppCompatEditText {

    private static final String TAG = "DEBUG-TCW: " + PasswordEditText.class.getSimpleName();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        // 大于Icon的位置, 才能触发点击(x<iconX非必要，如果添加响应区域只有icon除去了padding)
        if (x >= leftIcon && x < iconX) {
    ...
    public PointF getTopRightCorner() {
        ...
        float[] dst = new float[]{0, 0, getWidth() - getPaddingRight(), 0, 0, getHeight(), getWidth() - getPaddingRight(), getHeight()};
    ...
    private void showPasswordVisibilityIndicator(boolean shouldShowIcon) {
        ...
        try {
            ...
            bottomDrawable = getCompoundDrawables()[3];
        } catch (Exception e) {
        ...
        if (shouldShowIcon) {
            ...
            // 在最右侧显示图像
            setCompoundDrawablesWithIntrinsicBounds(leftDrawable, topDrawable, drawable, bottomDrawable);
